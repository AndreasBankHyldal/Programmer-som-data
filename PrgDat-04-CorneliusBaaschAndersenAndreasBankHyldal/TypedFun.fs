...
let rec typ (e : tyexpr) (env : typ env) : typ =
    ...
    | ListExpr(elements, elementType) -> 
      if List.forall (fun element -> typ element env = elementType) elements 
      then TypL elementType
      else failwith "ListExpr: element types differ" 
    | ListExpr _ ->
        failwith "eval: list expressions are only supported by the type checker"
